...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  try {
    ...
    try {
      ...
      const offers = (await nightmarketClient.GetOffers(mintPublicKey)).filter((offer: Offer) =>
        nightmarketClient.IsLocalOffer(offer)
      );

      if (offers.length === 0) {
        return res.status(404).json({ error: 'No offers from the night market' });
      }
