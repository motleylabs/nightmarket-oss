...
function Footer({ links }: FooterProps) {
  return (
    <footer className="mt-16 bg-gray-900 py-20 px-6 md:mt-28">
      <div
        className={clsx(
          `container mx-auto grid grid-cols-2 items-center gap-y-4 md:grid-cols-${links.length + 2}`
        )}
      >
        <div>
          {/* Logos */}
          <Link className="flex flex-row gap-2 whitespace-nowrap pb-6 text-2xl font-bold" href="/">
            <img
        ...
        <div
          className={clsx(
            `col-span-1 row-span-2 grid gap-2 md:row-span-1 md:gap-4 md:col-span-${links.length}`,
            `md:grid-cols-${links.length}`
          )}
        >
          {links.map((col, colKey) => {
            return (
              <div
                className={`flex flex-col flex-wrap items-end justify-center gap-${
                  links.length === 1 ? '10' : '2'
                } text-right text-white md:flex-row md:gap-10 md:text-left`}
                key={`footer-col-${colKey}`}
              >
                {col.map((link, linkKey) => {
                  return (
                    <Link
                      className="w-[125px] hover:underline"
        ...
        <div className={clsx('flex gap-6 text-[#A8A8A8] md:justify-end')}>
          {/* Social media */}
